...
mkdir build
cd build
cmake .. -DCMAKE_OSX_ARCHITECTURES=i386 -DCMAKE_OSX_DEPLOYMENT_TARGET=10.6 -DCMAKE_OSX_SYSROOT=/Library/Developer/CommandLineTools/SDKs/MacOSX10.13.sdk -DFREETYPE_PNG=ON -DFREETYPE_ZLIB=ON -DSHARED=OFF -DWANT_NATIVE_IMAGE_LOADER=OFF -DWANT_DEMO=OFF -DWANT_EXAMPLES=OFF -DWANT_TESTS=OFF -DWANT_VIDEO=OFF -DWANT_IMAGE_FREEIMAGE=OFF -DZLIB_INCLUDE_DIR=/usr/local/include -DZLIB_LIBRARY_RELEASE=/usr/local/lib/libz.a
make
...
mkdir build
cd build
cmake .. -DCMAKE_OSX_ARCHITECTURES=x86_64 -DCMAKE_OSX_DEPLOYMENT_TARGET=10.7 -DCMAKE_CXX_FLAGS=-stdlib=libc++ -DFREETYPE_PNG=ON -DFREETYPE_ZLIB=ON -DSHARED=OFF -DWANT_NATIVE_IMAGE_LOADER=OFF -DWANT_DEMO=OFF -DWANT_EXAMPLES=OFF -DWANT_TESTS=OFF -DWANT_VIDEO=OFF -DWANT_IMAGE_FREEIMAGE=OFF -DZLIB_INCLUDE_DIR=/usr/local/include -DZLIB_LIBRARY_RELEASE=/usr/local/lib/libz.a
make
...
mkdir build
cd build
cmake .. -DCMAKE_OSX_ARCHITECTURES=arm64 -DCMAKE_OSX_DEPLOYMENT_TARGET=10.7 -DCMAKE_CXX_FLAGS=-stdlib=libc++ -DFREETYPE_PNG=ON -DFREETYPE_ZLIB=ON -DSHARED=OFF -DWANT_NATIVE_IMAGE_LOADER=OFF -DWANT_DEMO=OFF -DWANT_EXAMPLES=OFF -DWANT_TESTS=OFF -DWANT_VIDEO=OFF -DWANT_IMAGE_FREEIMAGE=OFF -DZLIB_INCLUDE_DIR=/usr/local/include -DZLIB_LIBRARY_RELEASE=/usr/local/lib/libz.a
